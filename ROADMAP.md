...
## 🔊 Audio Realism Pass (2026-05-31) — agent-designed; Phase A BUILT (ear-tune pending)
...
- **A — Per-note velocity/volume CONSISTENCY (FOUNDATION) — ✅ BUILT 2026-05-31 (ear-tune pending).** Root causes (both confirmed in code): scattered magic scalars + no per-pitch normalization, and the bent-note WAF→oscillator lurch. Built: `WAF_VOICE_VOL` named per-role levels; `wafLoudnessTrim(midi)` perceptual tilt (+2.5 dB <150 Hz → flat → −1.5 dB >2 kHz) on every sampled voice; the bent-note oscillator fallback now level-matched (incl. trim) to the sampler it replaces (kept the audible slide — the chosen fork; oscillator-only profiles unchanged). Smoke-green (renderers exercise the audio path). **Numbers are first-pass — Christian to tune by ear next pass** (the loudness curve dB points + the bent-note match level).
- **B — WAF for ALL backing + synth FAILOVER.** Default every backing role to sampled (the existing per-voice load-then-synth fallback *is* the failover — formalize + widen it); distorted family's pad → a sampled comp; pad stays synth. Extend the pad's `0.5/√n` density gain-scaling to all WAF voices; bake the mix carve (HP ~120 Hz on comp, −2/−3 dB dip 1–2.5 kHz; levels player 0 / bass −5/−6 / comp −10/−12 / click −14).
- **C — Drop the backing-voice dropdown.** Remove `harmonyTone` (form + `readConfig`/`resolveAudioProfile`); add `mixerState[key].instrument` (GM program, default `null` = use profile, so removal regresses nothing). The mixer `<select>` UI is GUI build #6.
- **D — DRUMS (the realism unlock, last).** Self-host ~8–12 WebAudioFont GM **bank-128** percussion under `static/wafonts/` (zero route changes — the `/wafont` guard already serves them); add a `drums` bus + `role:'drums'` events; `AUDIO_PROFILES` → an ensemble spec. **Hearing-safety (hard, sound-design-owned):** 3–6 ms attack ramp per hit, a dedicated drum-bus compressor *before* the master, **never loosen the master limiter** (−6/12/6), **drums silent during count-in** (enter on the downbeat after the count), don't stack click+drum on beat 1. **Dependency:** the drum PATTERN per style/feel belongs to the rhythm-meter / genre-idiom lane — engine exposes the `role:'drums'` trigger; that lane authors the groove. Sequence the pattern work alongside D.
...
### ⏸️ STOPPED HERE (2026-05-31) — Audio Phase A committed; NEXT = ear-tune A, then B → C → D
Phase A (consistency) is built + committed, **pending Christian's ear-tune** of the loudness-curve dB points and the bent-note match level (it's by-ear; he chose to commit now and tune next pass). **Next:** tune A by ear → build **B** (WAF-for-all + failover) → **C** (drop dropdown + mixer instrument hook) → **D** (drums; needs the rhythm/genre drum-PATTERN handoff). The GUI build queue (#2→#7) and the prior drums/progress-store items remain open in parallel.
