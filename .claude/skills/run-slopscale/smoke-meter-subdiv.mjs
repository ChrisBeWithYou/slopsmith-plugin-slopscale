...
try {
  ...
  await page.waitForFunction(() => window.SlopScale && typeof window.SlopScale.readConfig === "function", { timeout: 10000 });
  ...
  const setMeter = (v) => page.evaluate((v) => {
    const m = document.querySelector('#slopscale-controls [name="meter"]');
    m.value = v; m.dispatchEvent(new Event("change", { bubbles: true }));
  }, v);
  const setSubdiv = (v) => page.evaluate((v) => {
    const s = document.querySelector('#slopscale-controls [name="subdivision"]');
    s.value = v; s.dispatchEvent(new Event("change", { bubbles: true }));
  }, v);
  const state = () => page.evaluate(() => ({
    subdiv: document.querySelector('#slopscale-controls [name="subdivision"]').value,
    meter: document.querySelector('#slopscale-controls [name="meter"]').value,
    help: (document.getElementById("slopscale-division-help") || {}).textContent || "",
  }));
  ...
  await page.evaluate(() => { const f = document.querySelector('#slopscale-controls [name="bpm"]'); f.value = "92"; f.dispatchEvent(new Event("change", { bubbles: true })); }); // a regenerate-y change
  s = await state();
