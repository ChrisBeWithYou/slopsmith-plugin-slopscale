...
*2026-06-13. Main-thread synthesis of a 3-lane design pass (slopsmith-host-expert · audio-engine-architect · slopscale-ux-designer). Christian: "if we're doing the amp/cab interface we don't just need an IR picker, we need more. Is there a NAM gui we can integrate or do we have to build our own?" + "look into A2." This is for SlopScale's **BACKING band** amp/cab tone (the generated comp/guide voices) — NOT the player's live input (that's the host `nam_tone` plugin).*

## The facts that reframed it
- **`nam_tone` (host plugin, v1.2.0)** is already a full NAM amp/cab rig — upload `.nam` models + cab IRs, build presets (model+IR+gain+gate), tone-map to songs, a config GUI, a SQLite preset DB, and a WASM NAM worklet served cross-plugin. But it amps the **player's LIVE input**. SlopScale's amp/cab is for the **backing** — same engine family, different signal.
...
- **Persistence:** IR bytes → IndexedDB (`slopscale-rig`), metadata (name + output) → localStorage (`slopscale.rig`). A user upload is THEIR licensing problem (never committed). core-purity stays green (Rig is shell, not the generation path).
...
Agent specs: `.claude/agent-memory/{slopsmith-host-expert/reference_nam_tone_borrow_surface, audio-engine-architect/project_backing_amp_method_and_a2, slopscale-ux-designer/project_amp_cab_interface_spec}.md`.
