...
usability LEAD/co-lead: `slopscale-ux-designer` + `gamification-architect` · voices:
`drum-/bass-/piano-pedagogy` · exemplars: `metal-/country-/jazz-idiom` · data model + progressions:
`harmony-theory-architect` · borrow: `slopsmith-host-expert` · context: `market-analyst`,
...
7. **Lighter/heavier sections are a density TIER, not a song-section flag** (metal's clean break = the
   `:sparse` tier). SlopScale generates practice loops, not arranged songs — the tier *is* the dynamic-
...
**Host-confirmed (slopsmith-host-expert, runtime 0.2.9 — the rule-4 HOST CHECK):** none of the realism fix
