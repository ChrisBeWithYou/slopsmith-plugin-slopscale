...
**Phased To-Do (M1–M16):**
- **Phase 0 — engine foundation (KEYSTONE):** **M1** add the bright-modal progression tokens to `COMMON_PROGRESSIONS` (unblocks palettes/Jam/most pathways at once) · **M2** quartal/So-What voicing primitive (So-What = 3 stacked 4ths + a maj-3rd on top) · **M3** `nonFunctional`/`modalColor` voicing intent (suppress the tritone, omit the leading tone).
