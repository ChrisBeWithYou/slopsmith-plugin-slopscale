...
- **CSS cleanup.** `.slopscale-caged-shapes`, `.slopscale-caged-shape-btn`, `.slopscale-caged-only` rules are still in the stylesheet but no markup uses them. Safe to remove in a follow-up.
...
1. Open SlopScale → pick **Chord Tone Targeting** → key **C** → the Shape dropdown should show `C-shape (frets 0–5)` selected by default. The Cmaj7 arpeggio bars should stay within frets 0–5; no notes wandering to fret 7.
