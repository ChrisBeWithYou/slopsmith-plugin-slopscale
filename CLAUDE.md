...
There is **no unit-test or lint suite**. Verification is behavioural, via ten Playwright smoke suites in the `run-slopscale` skill, run against a live host (start it with `launch.ps1` first):

- `npm test` (from `.claude/skills/run-slopscale/`) runs **all ten** suites — renderers, then generators, then highway-settings, then strings/tuning, then audiocontext-sharing, then chord-scale connect, then variation-engine, then session-sync, then depth-ladder/progress, then core-purity.
