# SlopScale Progression & XP Infrastructure — developer handover
...
SlopScale is a practice/learning tool. The whole layer follows one principle: **gamification describes mastery, it never substitutes for it and never gates content.** Concretely:
...
- **The XP mode switch is real.** `slopscale.progress.mode ∈ off | casual | hardcore`. **Off collapses the entire layer** — no XP, no depth rungs, no badges, no emits (the standing proof it never gates). `progressSetMode()` writes it; `slopscale.xpMode` stores the settings-panel default.
...
|---|---|---|
| `slopscale.pathway_tiers` | `{ [pathwayId]: { highest_tier, bestBpm?, feltBpm? } }` | **The competency ledger** — the Speed axis (`highest_tier` = highest tempo tier cleared, indexes `TIER_LABELS = ['Slow','Med','Fast','Push']`), the guitar clean-tempo PB (`bestBpm`), and the bass pocket PB (`feltBpm` = highest tempo held *Locked*). |
| `slopscale.progress` | `{ mode, xp, byNode: { [id]: { reps, depth:{ travel, clean, eyesOff }, masteredAt, keysCleared[] } }, badges: { [id]: ts } }` | **The depth/XP store** (`PROGRESS_KEY`). `xp` is a derived number (see §4). `depth.*` are one-time timestamp flips; `eyesOff` is reserved/unused. `keysCleared` backs the Travel axis. |
| `slopscale.sessions` | array, newest first | **The session log** — every run ≥2s (`sessionBegin`/`sessionEnd`): mode, pathway_id, bpm/bpm_tier, scale, key (+ `key_credit`/`tuning_offset` — see Travel below), hit/miss counts, duration. Feeds the woodshed log, streaks, recognizers. |
| `slopscale.spec_best` | `{ [specKey]: { best, runs } }` | The "Best here" standing-target line (per exact exercise spec). |
| `slopscale.proofloop` | flag | The proof-loop pilot gate (`proofPilot()` — pilot pathways only). |
| `slopscale.xpMode` | string | Settings default for the mode switch. |

Both presets and tunings live server-side in the Slopsmith meta-DB; **everything above is client-local**. There is no server sync of progress today.

## 2 · The credit front doors (the ONLY writers of the ledger)

All tier writes funnel through **`_updatePathwayTier(pathwayId, tier)`** — idempotent (flips only an uncleared tier), **MERGE-writes** (never drops sibling fields like `bestBpm`/`feltBpm`), and emits `slopscale:tier:unlocked` to the host. Four callers:
...
Outbound only, fire-and-forget, the store never depends on it: `window.slopsmith.emit('slopscale:tier:unlocked', {pathway, tier, label})` on a flip, and `emitProgress(kind, nodeId, extra)` → `'slopscale:progress'` events (`kind ∈ xp | depth | felt`). No host XP intake exists yet; these define the shape for when it does. Registration as a Slopsmith *minigame* was considered and declined (SlopScale is a practice studio, not a score-chaser — see the 2026-06-03 decision in `ROADMAP.md`).
