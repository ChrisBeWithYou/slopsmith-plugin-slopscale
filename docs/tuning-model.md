# How SlopScale models and names tunings

> Written up for the host-team discussion about tunings displaying as raw offset
> arrays (`-1 -1 -1 -1 -1 -1`) instead of names. SlopScale already ships a
...
shared meta-DB (table `slopscale_tunings`), upserted by name, listed per
...
- `.claude/skills/run-slopscale/smoke-strings.mjs` — the behavioural guard
