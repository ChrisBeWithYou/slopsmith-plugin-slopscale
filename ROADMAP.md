...
- 🔲 **Restore the built-in 2D Highway (string-name gutter) — UX decision.** The view with a left nut/string-name column (open-string note names) + lane layout is SlopScale's own `makeBuiltin2DRenderer` (`drawStringLanes` → `stringLabelForMidi`). It was demoted to a **last-resort fallback** when the `builtin_2d` slot switched to borrow the host **Jumping Tab** plugin (and the default to host **3D Highway**). Consequence: string-name labels only appear in our in-tree renderers (Tab/Notation/the dormant 2D highway); the host viz (3D Highway, Jumping Tab) don't draw that gutter — which is why the "string view" looks muted there. Options: (a) point `builtin_2d` back at `makeBuiltin2DRenderer`, (b) add it as a separate selectable view, (c) leave as fallback. `slopscale-ux-designer`'s call. This was NOT changed today — the host-viz-borrow switch predates this session.
- ℹ️ **3D highway "fret numbers at top / missing nut column" is host-side, not ours.** The host `highway_3d` plugin is unchanged since 2026-05-08; SlopScale's `syncHighwaySettings` only sets inverted/lefty/renderScale. The rolling fret-number row = highway_3d's **"Fret-column marker cadence"** setting (default 4; set 0 to disable) + the static inlay labels; both toggle in the 3D Highway plugin's own settings. The `run-slopscale` launched host uses a separate config dir (`C:\Users\chris\slopsmith-config`), so it shows highway_3d **defaults** rather than your Desktop-app preferences — likely the whole "it changed" perception. Nothing to revert in our repo.
