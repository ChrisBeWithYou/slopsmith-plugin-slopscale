...
Implementation: toggling adds/removes the class `slopscale-session-mode` on `#slopscale-root`. CSS rules hide `.slopscale-single-only` in session mode and `.slopscale-session-only` in single mode.
...
- `#slopscale-session-audio-notes` → inject `audio.notes` into each segment config
- `#slopscale-session-audio-metronome` → inject `audio.metronome`
- `#slopscale-session-audio-harmony` → inject `audio.harmony`
...
- **Saved sessions**: the `#slopscale-session-saved-group` optgroup is reserved for user-saved sessions via the preset CRUD routes. Not yet wired.
