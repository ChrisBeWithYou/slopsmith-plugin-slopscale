...
try {
  ...
  const gen = (patch) => page.evaluate((p) => {
    const cfg = Object.assign(window.SlopScale.readConfig(), {
    ...
    try {
      const e = window.SlopScale.generateExercise(cfg);
  ...
  const wired = await page.evaluate(() => {
    const ptOpts = Array.from(document.querySelectorAll('#slopscale-controls [name="practiceType"] option')).map(o => o.value);
    const pwOpts = Array.from(document.querySelectorAll('#slopscale-pathway option')).map(o => o.value);
    const ps = document.getElementById("slopscale-pathway");
    ps.value = "pick_herta"; ps.dispatchEvent(new Event("change", { bubbles: true }));
    const title = (document.getElementById("slopscale-pathway-title") || {}).textContent || "";
    const cfg = window.SlopScale.readConfig();
    let pwNotes = 0; try { pwNotes = (window.SlopScale.generateExercise(cfg).chart.notes || []).length; } catch (_) {}
