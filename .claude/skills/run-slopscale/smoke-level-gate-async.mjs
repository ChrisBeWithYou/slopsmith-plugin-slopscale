...
// Installed before the SlopScale run: an ASYNC desktop bridge getLevels (the
...
// contained API → SlopScale takes the setVerifyTarget fallback). A page-side
...
const INSTALL = () => {
  ...
  window.noteDetect = {
    ...
    // NO isContainedVerifierAvailable → SlopScale uses the setVerifyTarget fallback.
  };
...
try {
  ...
  await page.waitForFunction(() => window.SlopScale && typeof window.SlopScale.makeBundle === "function" && globalThis.__ss_debug && globalThis.__ss_debug.lvlMode);
  ...
  await page.evaluate(() => { document.getElementById("slopscale-stop")?.click(); if (window.__phantom) clearInterval(window.__phantom); });
