...
- **Toggle:** ONE pill ("Fingering"), view-bar after Fretboard view, default ON, persisted (`slopscale.showHandMarks`), never auto-flipped. Band scaling happens content-side (emission policy), not by flipping the user's switch; the **Clean depth rung** is the player-opted supports-off proving run (its first concrete mechanic).
