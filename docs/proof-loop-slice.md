...
> SlopScale's #1 priority (five independent market convergences). ONE build, behind a flag,
...
> **Status: BUILT + VERIFIED (2026-06-03).** Flag `localStorage['slopscale.proofloop']='on'`
...
## 4. Flag + scope
`localStorage['slopscale.proofloop']` (default **off**), gated by `PROOF_PILOTS = new Set([<pilot>])`.
