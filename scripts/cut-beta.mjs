#!/usr/bin/env node
/*
 * cut-beta.mjs — publish the "SlopScale (Beta)" build.
 *
 * WHAT IT DOES
 *   Builds a RENAMED mirror of `slopscale-dev` (plugin id `slopscale_beta`,
 *   name/label "SlopScale (Beta)", version `X.Y.Z-beta.N`) and commits it to the
 *   `slopscale-beta` branch. Because the id differs, the beta INSTALLS ALONGSIDE
 *   the stable SlopScale in Slopsmith — two nav entries, fully independent
 ...
 *   every plugin screen into one shared page, so two `slopscale` installs would
 *   collide (registration + duplicate DOM ids). See docs/beta-testing.md.
 *
 * WHY A SCRIPT (not a hand-maintained fork)
 *   screen.html/screen.js carry ~1600 literal `slopscale` references (element
 *   ids, /api/plugins/slopscale/ URLs, localStorage keys). Renaming by hand on
 *   every dev change is unmaintainable; this regenerates it mechanically.
 *
 * SOURCE = the latest COMMIT on the source ref (default slopscale-dev), NOT your
 *   working tree. Commit your dev work first.
 *
 * USAGE
 *   node scripts/cut-beta.mjs                 build + commit to local slopscale-beta (no push)
 *   node scripts/cut-beta.mjs --push          also push origin/slopscale-beta
...
const SRC_TOKEN = 'slopscale';        // lowercase id/namespace token
const BETA_TOKEN = 'slopscale_beta';  // underscore → valid SQL table names + valid DOM ids
const BETA_BRANCH = 'slopscale-beta';
...
const PUSH = has('--push');
const SOURCE = optVal('--source', 'slopscale-dev');
...
function writeTree(root) {
  ...
  pj.name = 'SlopScale (Beta)';
  pj.version = betaVersion;
  if (pj.nav) {
    if (pj.nav.label) pj.nav.label = pj.nav.label.replace('SlopScale', 'SlopScale (Beta)');
  ...
  for (const p of TRANSFORM) {
    ...
    if (p === 'screen.js') {
      s = s.replace(/(const\s+SLOPSCALE_VERSION\s*=\s*)(['"]).*?\2/, `$1'${betaVersion}'`);
    }
    out(root, p, s);
  }
  ...
  out(root, '.gitignore', `# Generated SlopScale (Beta) build — see scripts/cut-beta.mjs\n.${BETA_TOKEN}-temp/\n`);
}
...
// ── REAL: build the slopscale-beta branch in a throwaway worktree ──────────
...
try {
  ...
  if (!changed) {
    console.log(`[cut-beta] slopscale-beta already matches ${SOURCE} @ ${srcSha} — nothing to do.`);
  } else {
    const msg =
      `beta: sync slopscale-dev @ ${srcSha} (v${betaVersion})\n\n` +
      `Auto-generated SlopScale (Beta) build — a renamed (id: ${BETA_TOKEN}) mirror of\n` +
      `${SOURCE} so it installs ALONGSIDE the stable SlopScale. Do not edit by hand;\n` +
...
console.log(`\nTesters install ALONGSIDE stable (into the Slopsmith plugins dir):`);
console.log(`  git clone -b ${BETA_BRANCH} ${url} ${BETA_TOKEN}`);
console.log(`Update:  git -C ${BETA_TOKEN} pull   (then restart Slopsmith)`);
