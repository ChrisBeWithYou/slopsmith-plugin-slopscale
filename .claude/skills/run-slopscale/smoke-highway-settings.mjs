// Assertive smoke: SlopScale must never trip the host 3D-highway's viz
...
// settings panel owns. SlopScale must only READ that look, never WRITE it —
...
//   node .claude/skills/run-slopscale/smoke-highway-settings.mjs
...
async function main() {
  ...
  try {
    ...
    await page.waitForSelector("#slopscale-root", { state: "attached", timeout: 10000 });
    await page.waitForSelector(".slopscale-view-btn", { timeout: 5000 });

    // Ensure the 3D highway is the active view, then settle.
    const hwBtn = await page.$('.slopscale-view-btn[data-renderer="highway_3d"]');
    ...
    const play = await page.$('#slopscale-play, [data-action="play"], .slopscale-play');
    if (play && await play.isVisible()) await play.click();
    await page.waitForTimeout(2500);
    // Stop (same control toggles, or Stop button).
    const stop = await page.$('#slopscale-play, [data-action="play"], .slopscale-play, [data-action="stop"]');
    ...
    for (const kind of ["builtin_2d", "tab_2d", "notation_2d", "highway_3d"]) {
      const b = await page.$(`.slopscale-view-btn[data-renderer="${kind}"]`);
    ...
    for (const k of keys) {
      if (!(k in before)) failures.push(`SlopScale CREATED host setting ${k}=${after[k]}`);
      else if (!(k in after)) failures.push(`SlopScale DELETED host setting ${k} (was ${before[k]})`);
      else if (before[k] !== after[k]) failures.push(`SlopScale MUTATED host setting ${k}: ${before[k]} -> ${after[k]}`);
    }
    ...
    if (failures.length === 0) {
      console.log("PASS  highway-settings: SlopScale tripped no host h3d_bg_* setting");
    } else {
