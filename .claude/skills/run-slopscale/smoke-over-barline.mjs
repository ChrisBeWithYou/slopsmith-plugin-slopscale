...
try {
  ...
  // 1) option present
  const opts = await page.evaluate(() => Array.from(document.querySelectorAll('#slopscale-pathway option')).map(o => o.value));
  ...
  const sel = await page.evaluate(() => {
    ...
    const subField = document.querySelector('#slopscale-controls [name="subdivision"]').value;
    const meterField = document.querySelector('#slopscale-controls [name="meter"]').value;
