...
**For SlopScale:** The guide tone generator produces `s` and `f` values for
...
**For SlopScale:** Add `practiceType: 'pent_superimposition'` as an advanced
...
**SlopScale application:** A "Coltrane Changes" exercise generates arpeggio
...
**For SlopScale:** When a pathway uses tritone subs, the `chordOverride` system
...
**For SlopScale:** The scale generator, when targeting a maj7 or dom7 chord,
...
**For SlopScale:** Add a `modalVamp` exercise type — one chord, one scale,
...
**For SlopScale:** The "outside" vocabulary is Phase 3+ territory — it requires
...
**For SlopScale:** Quartal voicings are a chord/comping exercise type, not a
single-note line exercise. Their application in SlopScale is in the comping
...
1. **Listen first.** Know the original recording before playing a note.
   (SlopScale can't enforce this but can prompt it.)
...
**This sequence IS the SlopScale Practice Session template for a jazz pathway.**
