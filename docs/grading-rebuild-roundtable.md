# Grading Rebuild Roundtable — kill the false-positive judge, follow Slopsmith's example

*2026-06-08. Christian's dogfood report: "false positives with the new notedetect bundled in Slopsmith — remove anything we do that judges pitch/timing and rebuild from the ground up, borrowing/using what we can from the host. Grading should tie into and PULL FROM the Slopsmith system, not replace or augment (for now)." His question: "How is Slopsmith grading songs, and shouldn't we follow their example?"*

*Nine-lane group-design panel (slopsmith-host-expert, audio-engine, rhythm-meter, harmony-theory, guitar-pedagogy, bass-pedagogy, learning-design [content chair], gamification, slopscale-ux) + a two-lane technical re-consult (audio-engine, host-expert) after Christian's hardware correction. Per-lane rulings live in each `.claude/agent-memory/<agent>/`; this doc is the synthesis, the diagnosis, the build spec, and the decision log.*
...
**Interface-agnostic principle:** SlopScale **follows the host for ALL sound input/output**. The fix reads the host's own input meter (`getLevels()`), which reports whatever interface/device the player configured in Slopsmith — so it accommodates the full range of interfaces Slopsmith supports without naming, assuming, or special-casing any device.
...
- **Desktop (the real player base):** poll `window.slopsmithDesktop.audio.getLevels()` → `{inputLevel,inputPeak}` ~50ms. This reads the **host JUCE engine input — whatever interface/device the player configured in Slopsmith** — the SAME source the verifier scores, so we stay interface-agnostic by following the host. (A `getUserMedia` tap on desktop would instead grab the OS-default capture device, NOT the host-configured interface — wrong source.) ~20 IPC/s, trivial.
...
- **Prune the augmentation-beyond-host:** cut our bespoke latency **calibration sweep** (`ptCalibrateOffsetMs`/`slopscale.latencyMs`) → read the host's `latencyOffset`/`av_offset_ms` (static 80ms / inherited slider); cut **tremolo span credit** → one onset window; `ptSpeakBudget` → keep as a classifier, cut as a window widener; add `!n._tail` to the judged-set filter; thin paint to the **strip reward-ring** now, **defer (not delete)** Tab/Notation/2D paint until the gate is dogfood-trusted; demote timing-tendency/ms analytics to dev-verbose; **keep** the "Judging N of M" line, exemption rows, and **elevate the near-miss aggregate** (the humaneness valve under precision bias).
...
Shipped in `screen.js` (uncommitted on `slopscale-dev`), exactly per the spec above:
...
**The red-net scare was NOT this fix and NOT stale suites** (an earlier mis-diagnosis): the test host had **two SlopScale plugins loaded at once** — the dev `slopscale` (junction) AND `slopscale_beta` (the published beta, which Christian dogfoods from the bundled Program Files install + a stray `plugins-dev/slopscale_beta`). They collide (duplicate control ids; `window.SlopScale` binds to whichever loads last), so the suites drove one plugin while reading the other → 8 suites red. Fix: removed `plugins-dev/slopscale_beta` and test on the **checkout** runtime (which doesn't load the bundled beta), leaving Christian's dogfood-beta untouched. **Operability gotcha:** `SLOPSMITH_SOURCE=bundled` will re-collide while the beta lives in Program Files — use checkout mode for the net (or teach `launch.ps1` to skip `slopscale_beta`).
...
- Christian's hardware correction (a direct interface/DI signal + headphones + active gate, no mic) **refuted the acoustic-bleed theory** (audio-engine's initial lead); the lead-mute / Listen-vs-Play fork is OFF the table. SlopScale follows the host for all sound I/O, so the fix is device-agnostic across every interface Slopsmith supports. Re-consult re-rooted the cause to the scale-invariant comb firing on an inaudible residual through the gate-stripped verify side-door.
