...
### ⏸️ STOPPED HERE (2026-05-31) — Audio A + B + C built; ear-tune PINNED by Christian; NEXT = D (drums)
Phase A (consistency) + the WAF-load-swap fix + Phase B (WAF-for-all-backing + kept failover + density-scaling) + Phase C (backing-voice selection moved to the Mixer as per-channel instrument selects) are built + committed. **Ear-tune is PARKED at Christian's request** — the by-ear numbers (A's loudness-curve dB points + bent-note match level; B's mix-carve HP/dip + relative levels) wait for a dedicated listening pass; he's NOT hearing-sensitive (see `feedback_audio_safety_not_personal_sensitivity`), the bar is just normalized/standard/safe levels. **Next:** **D** (drums — self-host GM bank-128 percussion + a `drums` bus + ensemble `AUDIO_PROFILES`; needs the rhythm/genre drum-PATTERN handoff + a safe drum-bus compressor before the master). Also open in parallel: the **mixer-growth GUI #6** (master channel + resize + vertical strips — design-system §11), the rest of the **GUI build queue (#2→#7)**, and the prior **progress-store** item.
