...
## 🔊 Audio Realism Pass (2026-05-31) — A/B/C + D1–D3 BUILT (ear-tune pending); D4–D6 (sampled kits + humanization) next
...
- **D — DRUMS (the realism unlock, last) — ◐ GROUP-DESIGNED + D1–D3 BUILT 2026-05-31 (uncommitted).** 4-agent session (audio-engine · sound-design · rhythm-meter · drum-pedagogy); main thread synthesized + built the zero-asset synth path. Full spec: project memory `project_drums_phase_d_spec` + the four `.claude/agent-memory/*/…drum…` files. **CORRECTION to the old plan:** do **NOT** ship generic GM **bank-128** as the sound (that's the cheesy kit). New direction: **synth-808/909 (procedural Web Audio, zero-asset, zero-licensing, doubles as the never-silent failover) for electronic genres; curated CC0 WAF one-shots for acoustic; the KIT changes per genre** (`kit` field on the ensemble `AUDIO_PROFILES`).
   ...
   - **Build queue:** ✅ **D1** engine plumbing + synth kit + straight groove · ✅ **D2** full 808/909 recipes + four_on_floor/half_time + shuffle_blues · ✅ **D3** KIT_REGISTRY + ensemble AUDIO_PROFILES + selection + mixer Drums/kit picker — **all BUILT 2026-05-31 (uncommitted)** · **D4** sampled CC0 acoustic kits (LICENSING-gated; uncleared → gitignored `static/drums-pending/`; needs `wafDrumVoice` = `wafVoice`−`wafLoudnessTrim`) + swing_jazz brush/ride · **D5** humanization (ghost/velocity/micro values from drum-pedagogy) + reggae one_drop/NOLA second_line/metal double-kick/funk_16th grooves + fills (`fillEvery`) · **D6** kit_latin/kit_afro (heaviest + genre-agent patterns) · reverb send · opt kit_metal.
...
### ⏸️ STOPPED HERE (2026-05-31) — Audio A+B+C + Phase D1–D3 (synth drums) BUILT; NEXT = D4 (sampled kits) + D5 (humanization)
Phase A/B/C committed; **Phase D1–D3 (the zero-asset synth-drum path) BUILT this session, UNCOMMITTED** — see the Phase D bullet above for the per-symbol detail. Christian said "commit to D1–D3, fix on the backend if anything's wrong"; built + smoke-green (64/64 gen · 4/4 rend · highway-settings) + drums probe. **Ear-tune PARKED** (A/B numbers + now the drum per-piece gains/levels) for one dedicated listening pass — safe/normalized only (`feedback_audio_safety_not_personal_sensitivity`). **NEXT:**
- **D4 — sampled CC0 acoustic kits** (kit_rock/kit_jazz/kit_acoustic_soft): source curated CC0 one-shots → flat `static/wafonts/*.js` presets (uncleared → gitignored `static/drums-pending/`); add `wafDrumVoice()` = `wafVoice` **minus `wafLoudnessTrim`** + a sampled branch in `scheduleDrumHit`/`resolveDrumKit` (KIT_REGISTRY entries gain `engine:'sample'`); register the kits so the family-default `kit_rock` stops falling back to 909. **Gated on F1 (acoustic sourcing fork — recommended: curated CC0).**
- **D5 — humanization + per-genre grooves:** fill the `velocity`/`ghost`/`micro` hooks with drum-pedagogy's ranges (acoustic only; electronic stays tight); add reggae `one_drop`, NOLA `second_line`, metal double-kick, `funk_16th`; fills via `fillEvery`.
- **D6 + follow-ons:** kit_latin/kit_afro (genre-agent patterns) · reverb/ambience send (F7, carry-over from B) · opt kit_metal.
