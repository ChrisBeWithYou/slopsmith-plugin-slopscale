...
This document grounds SlopScale's feature design in established guitar pedagogy. It exists so that as SlopScale grows, every new generator/exercise has an explicit pedagogical reason to exist, and the practice-routine UX maps cleanly onto how serious players actually train.

It is also a teaching reference — beginners reading SlopScale's docs should be able to understand *why* the plugin generates what it generates.
...
| Skill | Why | Automatable in SlopScale |
|-------|-----|--------------------------|
...
| Open chord transitions (C/G/Am/F/Em/D) | Rhythm foundation | Not the SlopScale focus — Slopsmith handles songs |
...
### Rocksmith "Riff Repeater" pattern (BPM ladder)
Loop the figure at 60% tempo, auto-bump 5 BPM each clean pass, until target. SlopScale can render this statically as a chart whose tempo *steps up* over time, marked by section headers. (Slopsmith's scoring would enable the true "pass to advance" version later.)

### Master Mode pattern (memorization)
Rocksmith's Master Mode fades notes out as you master a section. SlopScale equivalent: generate the same routine N times, removing visual notes from each successive section while keeping the click. Trains internalization.

## How this maps onto SlopScale's generator
...
| **Master Mode fade** | Visual-only post-pass | Rocksmith memorization | Low — manipulates `notes[]` |
...
Each segment generates its own chart fragment. The session is concatenated into one temp Sloppak with section markers per segment so Slopsmith's section navigation shows the structure.
...
- **Vibrato quality** — needs mic input + pitch-tracking, belongs to Slopsmith's scoring layer, not the chart generator.
- **Tone / amp setup** — out of scope; Slopsmith Desktop handles tone.
- **Rhythm strumming patterns** — Slopsmith owns rhythm tracks; SlopScale is single-note-line focused.
- **Sight-reading from staff notation** — different rendering, different problem.
- **Ear training** — different UX entirely; consider as a future sibling plugin (`SlopEar`?), not inside SlopScale.
