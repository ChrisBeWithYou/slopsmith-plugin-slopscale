...
- **`main` HEAD is a PUBLISHED end-user artifact** (since Slopsmith Desktop v0.2.9, 2026-06-08): SlopScale is a bundled host plugin AND the host's `update_manager` zips our default-branch HEAD when a Desktop user updates (the user-dir copy wins over the bundled snapshot). So **keep WIP on a branch; only land on `main` when green**, and **bump `plugin.json` version on every shippable change** (the host caches by id+version). On a NEW Slopsmith release, run the **host-release overlap sweep** (ROADMAP standing ritual) before building — it's the recurring counterpart to the per-feature HOST CHECK in "Agent workflow" rule 4. Full mechanics: project memory `project_host_release_v029_audit`.
