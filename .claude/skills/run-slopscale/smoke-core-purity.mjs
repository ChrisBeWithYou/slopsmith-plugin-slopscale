...
// generation path (window.SlopScale.generateExercise / generateSession → a plain
// { version, session, chart }) does NOT reach for host/DOM globals — that chart is
// SlopScale's portable IP. makeBundle() is the chart→renderer-bundle BOUNDARY (it
...
//   node .claude/skills/run-slopscale/smoke-core-purity.mjs   # or: npm run smoke:core
//
...
function runPurityInPage(practiceTypes) {
  ...
  if (!S || typeof S.generateExercise !== "function") return { ok: false, reason: "window.SlopScale.generateExercise missing" };
  ...
  try {
    ...
    // BOUNDARY (§10), not core. It reads SlopScale display prefs (highway
...
async function run() {
  ...
  console.log("\n=== SlopScale core-purity smoke (host-independence) ===");
  if (!result || !result.ok) {
