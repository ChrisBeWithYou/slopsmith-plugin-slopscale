...
### ⏸️ STOPPED HERE (2026-05-31) — Audio A + B built; ear-tune PINNED by Christian; NEXT = C, then D
Phase A (consistency) + the WAF-load-swap fix + Phase B (WAF-for-all-backing + kept failover + density-scaling) are built + committed. **Ear-tune is PARKED at Christian's request** — the by-ear numbers (A's loudness-curve dB points + bent-note match level; B's mix-carve HP/dip + relative levels) wait for a dedicated listening pass; he's NOT hearing-sensitive (see `feedback_audio_safety_not_personal_sensitivity`), the bar is just normalized/standard/safe levels. **Next:** **C** (drop the `harmonyTone` dropdown + add `mixerState[key].instrument` hook, default null = use profile) → **D** (drums; needs the rhythm/genre drum-PATTERN handoff + a hearing-safe drum-bus). The GUI build queue (#2→#7) and the prior drums/progress-store items remain open in parallel.
