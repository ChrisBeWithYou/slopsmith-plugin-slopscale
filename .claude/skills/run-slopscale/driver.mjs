#!/usr/bin/env node
// SlopScale plugin driver.
//
// Launches the Slopsmith host with the plugin junctioned into its plugin dir,
// drives the SlopScale screen with Playwright, and takes screenshots.
//
// Usage (run via "npx playwright" so deps resolve):
//   node .claude/skills/run-slopscale/driver.mjs smoke
//   node .claude/skills/run-slopscale/driver.mjs screenshot <renderer>
...
//   - Plugin junctioned at %LOCALAPPDATA%\Slopsmith\plugins\slopscale
...
async function gotoSlopScale(page) {
  // Slopsmith is an SPA — load the shell, then wait for loadPlugins() to
  // inject the plugin's <section id="slopscale"> into the DOM, then call
  // showScreen() to activate it. plugin.json declares nav.screen = "slopscale".
  await page.goto(`${HOST}/`, { waitUntil: "domcontentloaded" });
  // Slopsmith mounts each plugin screen under #plugin-<id>. The plugin's own
  // root markup (#slopscale-root, from screen.html) lives inside that.
...
async function generate(page) {
  ...
  // Custom mode does expose #slopscale-regenerate; fall back to that when
  ...
  if (tier && await tier.isVisible()) {
    ...
    // Force a regenerate via the public API exposed on window.SlopScale.
    await page.evaluate(() => window.SlopScale?.generateExercise?.(window.SlopScale?.readConfig?.() || {}));
  }
