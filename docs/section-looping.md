...
## What Slopsmith does (reference implementation)

Slopsmith's looping lives in `static/app.js` and is exposed to plugins via
...
## How this maps to SlopScale

SlopScale runs its **own** transport (`tick()` + `currentPracticeTime` +
...
So SlopScale needs its own A-B loop layer with the same shape as the host's,
but plugged into SlopScale's own clock.
...
3. **Public API on `window.SlopScale`:**

   ```js
   window.SlopScale.setSegmentLoop(a, b)   // commits loopA/B; seeks to A
   window.SlopScale.clearSegmentLoop()      // unsets both, no playhead jump
   window.SlopScale.getSegmentLoop()        // → {a, b}
   ```

4. **Event emission via `window.slopsmith.emit`** (when available):
   - `slopscale:loop:set` `{ a, b }`
   - `slopscale:loop:clear` `{}`
   - `slopscale:loop:wrap` `{ a, b, time: a }`

   Prefix is `slopscale:` not `loop:` so we never shadow or conflict with
...
- Saved-loops dropdown:
  - New backend route: `GET/POST/DELETE /api/plugins/slopscale/loops`,
    keyed by a hash of the exercise config (key+scale+pathway+shape) so
    saved loops survive a regenerate. (Slopsmith keys by `filename` —
    we don't have one.)
- Count-in toggle + rewind-animation toggle. Both are quality-of-life and
  worth shipping behind defaults that match Slopsmith's behaviour.
...
- **Saved-loop persistence** — Slopsmith persists per-song; we'd persist
  per-exercise-config. Different keying strategy. Deferred to phase 2.
- **JUCE / Electron audio path** — Slopsmith juggles HTML5 audio and the
  native JUCE backend; SlopScale only has Web Audio oscillators, so no
  equivalent.
- **`_countInGen` teardown generation counter** — necessary in Slopsmith
  because count-in spans multiple async steps that can race against song
  changes. SlopScale's wrap is synchronous in `tick()`, no races to guard.
...
### `window.SlopScale.setSegmentLoop(a, b)`
...
`slopscale:loop:set`. Returns `void`.

### `window.SlopScale.clearSegmentLoop()`

Drops both endpoints. Does **not** stop playback or move the playhead.
Emits `slopscale:loop:clear`.

### `window.SlopScale.getSegmentLoop()`
...
- `slopscale:loop:set` — fired after `setSegmentLoop` commits.
- `slopscale:loop:clear` — fired after `clearSegmentLoop` commits.
- `slopscale:loop:wrap` — fired each time `tick()` wraps from B back to A.
