...
### ⏸️ STOPPED HERE (2026-06-01) — extended-range CAGED/Open/sweep fix COMMITTED; NEXT = "playing-the-changes" Connect keystone (Stage 1); v0.5.1 SHIPPED
- **NEXT: "Playing-the-changes" framework — Connect keystone (Stage 1), decided by Christian after a 3-agent consult (harmony + guitar-pedagogy + learning-design, all converged).** The blind spot: our engine gets the notes right per chord but has ZERO navigation between them — `mode_of_moment` restarts the line on the chord root and jumps box every bar ("scale practice with a better click", exactly what Christian doesn't want). **Keystone (Stage 1, building now):** stop restarting at root — make the chord-scale line **resolve to the nearest guide/chord tone of the next chord** (reuse `nearestPositionForPc` ~2104, already does voice-leading for guide-tones/walking-bass), threading a `prevMidi` cursor through the chart; and rename the `chord_tone_emphasis`/`mode_of_moment` binary to the nameable **Park / Switch / Connect** dial (Connect = the new behavior). Beginner↔advanced = dials on ONE engine (connection / navigation / target / density / tempo / assist). DEFERRED to Stage 2: beat-based chord-event timeline (harmonic rhythm, anticipation, 2-chords/bar — a clean superset of the degree array), the full connector-cell vocabulary (approach/enclosure/slide/common-tone-pivot/cell-displacement), the `region` abstraction (box stops being a hard wall), a true `changes_in_position` mode. Reconciliation ruling: "nearest" = HAND-distance at beg/int (playability=transfer), PITCH-nearest unlocks at advanced (tier-gated sub-dial). Full proposals in agent-memory `harmony-theory-architect/project_playing_changes_timeline_proposal.md` + `learning-design-architect/project_playing_changes_ladder.md` (R0→R6 ladder) + guitar-pedagogy's connector-vocabulary review.
