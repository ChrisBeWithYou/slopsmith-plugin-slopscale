...
2. **`feltHoldAnalyze(devs, { bpm, barSec })` → `{ verdict, leanMs, driftMs, jitterMs, n, spanBars }`** — a pure function (Theil-Sen slope, MAD×1.4826 detrended jitter, the gates, the bucketing, the §3 thresholds, sub-floor→null). Lives in the scorer section; expose on `window.SlopScale` for headless tests.
