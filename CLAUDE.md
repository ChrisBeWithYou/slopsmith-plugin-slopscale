...
There is **no unit-test or lint suite**. Verification is behavioural, via the Playwright smoke suites (every `smoke-*.mjs`) in the `run-slopscale` skill, run against a live host (start it with `launch.ps1` first):

- `npm test` (from `.claude/skills/run-slopscale/`) runs **every** suite — renderers, generators, highway-settings, strings/tuning, audiocontext-sharing, chord-scale connect, variation-engine, core-purity, progress (see `package.json` for the live list).
...
`screen.js` is one IIFE, ~11k lines (re-derive with `wc -l` — don't trust this figure). It loads as a classic `<script>` (the host injects it without `type="module"`), so it cannot use `import`/`export` — keep it one file. Major sections are marked with `§N` banner comments and indexed in a table-of-contents header at the top; **grep `§` to jump between sections, or use targeted `Grep` to locate one before reading.** Key sections (topical walk-through, **not** in §-order — see the TOC header in `screen.js` for the canonical §1–§15 order):
...
- **`BUILT_IN_SESSIONS`** — the starter Workout library: **11 multi-segment session presets** spanning goal × level × genre × instrument (a `starter_arc` warm-up→technique→application default that's the first key/select option + the `onLaunchSession` fallback; Beginner Foundations; ii–V–I Workshop; Daily 30-min Intermediate; Blues/Bebop Fundamentals; Warm-up 10-min; Rock Essentials; Funk Pocket; Metal Technique; **Bass Foundations**). Each uses only 3–4 of the ~29 primitives, leaving room for distinct user workouts. `onLaunchSession` inherits the form's `stringSetup` only within the same **instrument family** (so instrument-specific presets stay correct and guitar-only techniques never land on bass).
...
- **Gamification stays soft — no dark patterns (LOCKED, restated every charette).** Progress is opt-in and competence-framed: **only ever display XP *gained*, never "you lost X"**; **no streaks-as-pressure, calendars, daily-quota nags, or wall-clock "you owe N minutes" goals** (expose practice *depth* in rungs, not a clock). **XP is derived** (time × difficulty), gained-only, and **never gates** content. **Jam is a mirror, not a judge** — no score/combo/rank/leaderboard there. Loss-aversion is the dark pattern we refuse; competence-anticipation is the habit engine. When a feature could nudge via fear-of-loss, redesign it as a competence invite (the rejected skip-*penalty* survives only as a Hardcore opt-in *bonus*). Owned by `gamification-architect`.
- **GUI changes: edit the inline `<style>` in `screen.html` — there is no separate stylesheet** (the old `static/slopscale.css` is gone; don't recreate it), and **read `docs/design-system.md` first**. Preserve the wiring-contract IDs (`#slopscale-pathway`, the goal-card IDs, `#slopscale-tier-buttons`, `#slopscale-panel-toggle`, `#slopscale-save`) — `bind()` attaches without optional chaining, so **hide** elements per-mode, don't delete them. Don't conflate the **three** segmented-control families: `.slopscale-mode-bar` (→ `ss-mode-*`), `.slopscale-view-switcher` (renderer picker), and the stage's `.slopscale-modeview`/`.slopscale-focus-btn` (Setup/Play + Focus). There must always be exactly **one lit primary action** on first paint.
...
- Before launching a **large or token-expensive run** — full-corpus agent fan-out, source/PDF ingestion (e.g. the djent PDFs, ~1M tokens), or mass regeneration — **estimate the scope/cost and get sign-off first**; runs like these have been rejected for cost before. Default to a bounded snippet/sample pass and confirm before the full sweep.
...
- If conventions changed, keep `CLAUDE.md` and `AGENTS.md` **in sync** (they mirror) — edit both in the same pass. **Don't hard-code chase-able counts** (screen.js line count, number of smoke suites, `BUILT_IN_SESSIONS`/template/pathway counts): they drift and have needed repeated re-sync commits. Prefer self-describing phrasing ("every `smoke-*.mjs` suite", "the starter Workout library"); if you must cite a number, re-derive it from source (`wc -l`, `ls`) in that pass and write the same value to both files.
